...
public:
    ...
    void Create(std::string&& source, GLenum type, u64 hash) {
        shader_hash = hash;
        shader_type = type;
...
public:
    explicit TrivialVertexShader(bool separable) : program(separable) {
        program.Create(GenerateTrivialVertexShader(separable), GL_VERTEX_SHADER, 0);
    }
...
static std::size_t OGLProgramCacheSave(FileUtil::IOFile& file, u64 hash, OGLProgram& program) {
    GLenum format = 0;
    std::vector<GLbyte> binary;
    program.GetProgramBinary(format, binary);
    if (binary.empty()) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Save failed: empty binary! handle: {}",
                  program.handle);
        return static_cast<std::size_t>(-1);
    }
    ...
    // hash
    std::size_t total_size = 0;
    std::size_t data_size = sizeof(hash);
    std::size_t write_size = file.WriteBytes(&hash, data_size);
    if (write_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Save failed: hash!");
        return static_cast<std::size_t>(-1);
    }
    total_size += write_size;
    ...
    write_size = file.WriteBytes(&format, data_size);
    if (write_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Save failed: format!");
        return static_cast<std::size_t>(-1);
    }
    total_size += write_size;
    ...
    data_size = sizeof(binary_size);
    write_size = file.WriteBytes(&binary_size, data_size);
    if (write_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Save failed: binary size!");
        return static_cast<std::size_t>(-1);
    }
    total_size += write_size;
    ...
    write_size = file.WriteBytes(binary.data(), data_size);
    if (write_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Save failed: binary!");
        return static_cast<std::size_t>(-1);
    }
    total_size += write_size;
...
static std::size_t OGLProgramCacheLoad(FileUtil::IOFile& file, u64& hash, OGLProgram& program) {
    // hash
    std::size_t total_size = 0;
    std::size_t data_size = sizeof(hash);
    std::size_t read_size = file.ReadBytes(&hash, data_size);
    if (read_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Load failed: hash!");
        return static_cast<std::size_t>(-1);
    }
    total_size += read_size;
    ...
    GLenum format = 0;
    data_size = sizeof(format);
    read_size = file.ReadBytes(&format, data_size);
    if (read_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Load failed: format!");
        return static_cast<std::size_t>(-1);
    }
    total_size += read_size;
    ...
    data_size = sizeof(binary_size);
    read_size = file.ReadBytes(&binary_size, data_size);
    if (read_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Load failed: binary size!");
        return static_cast<std::size_t>(-1);
    }
    total_size += read_size;

    std::vector<GLbyte> binary(binary_size);
    data_size = binary_size;
    read_size = file.ReadBytes(binary.data(), data_size);
    if (read_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Load failed: binary!");
        return static_cast<std::size_t>(-1);
    }
    total_size += read_size;

    program.Create(format, binary);
    if (program.handle == 0) {
        LOG_DEBUG(Render_OpenGL, "OGLProgramCacheEntity Load failed: invalid!");
        return static_cast<std::size_t>(-1);
    }
...
static void OGLShaderCacheSave(bool separable, std::unordered_map<u64, OGLShaderStage>& shaders,
    ...
    // program count
    item_count = programs.size();
    data_size = sizeof(item_count);
    write_size = file.WriteBytes(&item_count, data_size);
    if (write_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLShaderCacheSave failed: program count!");
        return;
    }
    total_size += write_size;
    ...
    for (auto& iter : programs) {
        data_size = OGLProgramCacheSave(file, iter.first, iter.second);
        if (data_size == std::size_t(-1)) {
            LOG_DEBUG(Render_OpenGL, "OGLShaderCacheSave failed: program error!");
            return;
        }
        if (!file.IsGood()) {
            LOG_DEBUG(Render_OpenGL, "OGLShaderCacheSave failed: file is bad!");
...
static void OGLShaderCacheLoad(bool separable, std::unordered_map<u64, OGLShaderStage>& shaders,
    ...
    read_size = file.ReadBytes(&item_count, data_size);
    if (read_size != data_size) {
        LOG_DEBUG(Render_OpenGL, "OGLShaderCacheLoad failed: program count!");
        return;
    }
    total_size -= read_size;
    ...
    while (item_count > 0 && total_size > 0) {
        u64 hash = 0;
        OGLProgram program;
        data_size = OGLProgramCacheLoad(file, hash, program);
        if (data_size == std::size_t(-1)) {
            LOG_DEBUG(Render_OpenGL, "OGLShaderCacheLoad failed: program error!");
            return;
        } else {
            programs.emplace(hash, std::move(program));
        }
        if (!file.IsGood()) {
            LOG_DEBUG(Render_OpenGL, "OGLShaderCacheLoad failed: file is bad!");
            return;
...
public:
    ...
    bool UseProgrammableVertexShader(const Pica::Regs& regs, Pica::Shader::ShaderSetup& setup) {
        ...
        if (iter_ref == shaders_ref.end()) {
            ...
            if (vs_code.empty()) {
                ...
                if (new_shader) {
                    cached_shader.Create(std::move(vs_code), GL_VERTEX_SHADER, code_hash);
                }
                shaders_ref[key_hash] = &cached_shader;
                current_shader.vs = cached_shader.GetHandle();
                current_shader.vs_hash = code_hash;
            }
        } else if (iter_ref->second == nullptr) {
            current_shader.vs = 0;
            current_shader.vs_hash = 0;
        } else {
            current_shader.vs = iter_ref->second->GetHandle();
            current_shader.vs_hash = iter_ref->second->GetHash();
        }
    ...
    void UseFixedGeometryShader(const Pica::Regs& regs) {
        ...
        if (new_shader) {
            ...
            cached_shader.Create(std::move(gs_code), GL_GEOMETRY_SHADER, key_hash);
        }
        current_shader.gs = cached_shader.GetHandle();
        current_shader.gs_hash = key_hash;
    }
    ...
    void UseFragmentShader(const Pica::Regs& regs) {
        ...
        if (iter_ref == shaders_ref.end()) {
            ...
            OGLShaderStage& cached_shader = iter->second;
            if (new_shader) {
                cached_shader.Create(std::move(fs_code), GL_FRAGMENT_SHADER, code_hash);
            }
            shaders_ref[key_hash] = &cached_shader;
            current_shader.fs = cached_shader.GetHandle();
            current_shader.fs_hash = code_hash;
        } else {
            current_shader.fs = iter_ref->second->GetHandle();
            current_shader.fs_hash = iter_ref->second->GetHash();
        }
    }

    void UseTrivialVertexShader() {
        current_shader.vs = trivial_vertex_shader.GetHandle();
        current_shader.vs_hash = trivial_vertex_shader.GetHash();
    }

    void UseTrivialGeometryShader() {
        current_shader.gs = 0;
        current_shader.gs_hash = 0;
    }
    ...
    void ApplyTo(OpenGLState& state) {
        if (separable) {
            ...
            u64 shader_hashes[]{current_shader.vs_hash, current_shader.gs_hash,
                                current_shader.fs_hash};
            u64 hash = Common::ComputeHash64(&shader_hashes, sizeof(shader_hashes));
