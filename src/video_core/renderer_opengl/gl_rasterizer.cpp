...
struct DrawInfo {
    ...
    struct TevStage {
        Pica::TexturingRegs::TevStageConfig::Operation color_op;
        Pica::TexturingRegs::TevStageConfig::Operation alpha_op;
        Pica::TexturingRegs::TevStageConfig::Source color_source1;
        Pica::TexturingRegs::TevStageConfig::Source alpha_source1;
        Pica::TexturingRegs::TevStageConfig::ColorModifier color_modifier1;
        Pica::TexturingRegs::TevStageConfig::AlphaModifier alpha_modifier1;
        u32 color_multiplier;
        u32 alpha_multiplier;

        void Record(const Pica::TexturingRegs::TevStageConfig& tev) {
            color_op = tev.color_op;
            alpha_op = tev.alpha_op;
            color_source1 = tev.color_source1;
            alpha_source1 = tev.alpha_source1;
            color_modifier1 = tev.color_modifier1;
            alpha_modifier1 = tev.alpha_modifier1;
            color_multiplier = tev.GetColorMultiplier();
            alpha_multiplier = tev.GetAlphaMultiplier();
        }

        std::string ToString() const {
            return fmt::format("color_op: {}, alpha_op: {}, color_source1: {}, alpha_source1: {}, color_modifier1: {}, alpha_modifier1: {}, color_multiplier: {}, alpha_multiplier: {}",
                               color_op, alpha_op, color_source1, alpha_source1, color_modifier1, alpha_modifier1, color_multiplier, alpha_multiplier);
        }
    };
    ...
    void RecordTevStage(const Pica::Regs& regs) {
        tev_stage0.Record(regs.texturing.tev_stage0);
        tev_stage1.Record(regs.texturing.tev_stage1);
        tev_stage2.Record(regs.texturing.tev_stage2);
        tev_stage3.Record(regs.texturing.tev_stage3);
        tev_stage4.Record(regs.texturing.tev_stage4);
        tev_stage5.Record(regs.texturing.tev_stage5);
    }

    void DumpTevStage() {
        std::string log = fmt::format("TEV\n0: {}\n1: {}\n2: {}\n3: {}\n4: {}\n5: {}",
                tev_stage0.ToString(), tev_stage1.ToString(), tev_stage2.ToString(), tev_stage3.ToString(), tev_stage4.ToString(), tev_stage5.ToString());
        __android_log_print(ANDROID_LOG_INFO, "zhangwei", "%s", log.c_str());
    }
};
...
bool RasterizerOpenGL::AccelerateDrawBatchInternal(bool is_indexed) {
    ...
    if (current_frame != info.frame) {
        __android_log_print(ANDROID_LOG_INFO, "zhangwei", "total_draw： %d, primitive: %04X, is_indexed: %d, vertices: %d, light_enabled: %d, viewport: (%d, %d), draw: %d, frame: %d, cost: %ld",
                            info.total_draw, info.primitive_mode, info.is_indexed, info.num_vertices, info.light_enabled, info.viewport_x, info.viewport_y, info.draw, info.frame, info.time_cost);
        info.DumpTevStage();
        info.light_enabled = false;
        info.is_indexed = false;
        info.primitive_mode = 0;
        info.num_vertices = 0;
        info.frame = current_frame;
        info.draw = 0;
        info.total_draw = 0;
        info.time_cost = 0;
        info.viewport_x = 0;
    ...
    /*int64_t time_cost = ::Log::GetTimeNsec() - start;
    if (time_cost > info.time_cost) {
        info.light_enabled = !regs.lighting.disable;
        info.time_cost = time_cost;
        info.is_indexed = is_indexed;
        info.draw = info.total_draw;
        info.num_vertices = regs.pipeline.num_vertices;
        info.primitive_mode = primitive_mode;
        info.viewport_x = static_cast<u32>(Pica::float24::FromRaw(regs.rasterizer.viewport_size_x).ToFloat32()) * 2;
        info.viewport_y = static_cast<u32>(Pica::float24::FromRaw(regs.rasterizer.viewport_size_y).ToFloat32()) * 2;
        info.RecordTevStage(regs);
    }
    info.total_draw += 1;*/
        ...
        if (texture.enabled) {
            if (texture_index == 0) {
                ...
                case TextureType::Shadow2D: {
                    ...
                    if (surface != nullptr) {
                        if (color_surface && color_surface->texture.handle == surface->texture.handle) {
                            state.image_shadow_texture_px = surface->GetTextureCopyHandle();
                        } else {
                ...
                case TextureType::ShadowCube: {
                    ...
                    if (surface != nullptr) {
                        if (color_surface && color_surface->texture.handle == surface->texture.handle) {
                            state.image_shadow_texture_nx = surface->GetTextureCopyHandle();
                        } else {
                    ...
                    if (surface != nullptr) {
                        if (color_surface && color_surface->texture.handle == surface->texture.handle) {
                            state.image_shadow_texture_py = surface->GetTextureCopyHandle();
                        } else {
                    ...
                    if (surface != nullptr) {
                        if (color_surface && color_surface->texture.handle == surface->texture.handle) {
                            state.image_shadow_texture_ny = surface->GetTextureCopyHandle();
                        } else {
                    ...
                    if (surface != nullptr) {
                        if (color_surface && color_surface->texture.handle == surface->texture.handle) {
                            state.image_shadow_texture_pz = surface->GetTextureCopyHandle();
                        } else {
                    ...
                    if (surface != nullptr) {
                        if (color_surface && color_surface->texture.handle == surface->texture.handle) {
                            state.image_shadow_texture_nz = surface->GetTextureCopyHandle();
                        } else {
