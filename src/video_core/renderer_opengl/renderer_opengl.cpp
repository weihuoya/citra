...
void RendererOpenGL::DrawSingleScreenRotated(const ScreenInfo& screen_info, float x, float y,
    ...
    // screen_info.texture's width and height are not correct.
    float src_width = screen_info.texture.height * Settings::values.resolution_factor;
    float src_height = screen_info.texture.width * Settings::values.resolution_factor;
...
Core::System::ResultStatus RendererOpenGL::Init() {
    ...
    if (strcmp("Qualcomm", gpu_vendor) == 0 && strstr(gpu_model, "650")) {
