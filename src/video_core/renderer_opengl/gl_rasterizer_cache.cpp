...
Surface RasterizerCacheOpenGL::GetTextureSurface(const Pica::Texture::TextureInfo& info,
    ...
    if (params.is_texture) {
        ...
        } else if (params.addr == 0x180A6280 || params.addr == 0x180C6280) {
            // Fire Emblem - IF
            params.is_texture = false;
        }
    }
...
SurfaceSurfaceRect_Tuple RasterizerCacheOpenGL::GetFramebufferSurfaces(
    ...
    if (!using_depth_fb && Settings::values.texture_load_hack && color_params.addr < 0x181FFFFF) {
...
Surface RasterizerCacheOpenGL::CreateSurface(const SurfaceParams& params) {
    ...
    if (params.is_texture && params.width == 256 && params.height == 512 && params.pixel_format == PixelFormat::RGBA8) {
        // hack for display glitch
