...
void CachedSurface::UploadGLTexture(const Common::Rectangle<u32>& rect, GLuint read_fb_handle,
    ...
    if (pbo_size < gl_buffer_size || pbo_size < custom_tex_info.tex.size()) {
        pbo_size = std::max(Common::AlignUp(gl_buffer_size, 8), custom_tex_info.tex.size());
    ...
    if (is_custom) {
        ...
        glBufferSubData(GL_PIXEL_UNPACK_BUFFER, 0, custom_tex_info.tex.size(),
                        custom_tex_info.tex.data());
    ...
    if (surface == nullptr) {
        ...
        if (match_res_scale != ScaleMatch::Exact) {
            if (params.is_texture && params.width == 256 && params.height == 512) {
                // color palette texture, 256x512 or 512x1024
                target_res_scale = 1;
            } else {
...
Surface RasterizerCacheOpenGL::GetTextureSurface(const Pica::Texture::TextureInfo& info,
    ...
    params.is_texture = Settings::values.texture_load_hack && params.addr < 0x181FFFFF;
...
void RasterizerCacheOpenGL::FlushRegion(PAddr addr, u32 size, Surface flush_surface) {
    ...
    for (auto& pair : RangeFromInterval(dirty_regions, flush_interval)) {
        ...
        if (!GLES || surface->pixel_format < PixelFormat::D16) {
            if (surface->type != SurfaceType::Fill && (!surface->gl_buffer || !surface->is_texture)) {
                SurfaceParams params = surface->FromInterval(interval);
