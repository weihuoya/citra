...
FramebufferLayout SideFrameLayout(u32 width, u32 height, bool swapped) {
    ...
    if (height > width + width / 2) {
        ...
        u32 shift_vertical = (height - top_screen.GetHeight() - bot_screen.GetHeight()) / 3;
        res.top_screen = top_screen.TranslateY(shift_vertical);
        res.bottom_screen = bot_screen.TranslateY(shift_vertical + top_screen.GetHeight());
    } else {
