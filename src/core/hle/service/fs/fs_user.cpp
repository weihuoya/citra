...
void FS_USER::GetSdmcArchiveResource(Kernel::HLERequestContext& ctx) {
    ...
    u32 sectorSize = 4096;    ///< Size of each sector, in bytes.
    u32 clusterSize = 4096;   ///< Size of each cluster, in bytes.
    u32 totalClusters = 4 * 1024 * 1024; ///< Total number of clusters.
    u32 freeClusters = 4 * 1024 * 1024;  ///< Number of free clusters.

    IPC::RequestBuilder rb = rp.MakeBuilder(5, 0);
    rb.Push(RESULT_SUCCESS);
    rb.Push(sectorSize);
    rb.Push(clusterSize);
    rb.Push(totalClusters);
    rb.Push(freeClusters);

    LOG_DEBUG(Service_GSP, "FS_USER GetSdmcArchiveResource called");
}
...
void FS_USER::GetNandArchiveResource(Kernel::HLERequestContext& ctx) {
    ...
    LOG_DEBUG(Service_GSP, "FS_USER GetNandArchiveResource called");
}
...
void FS_USER::GetArchiveResource(Kernel::HLERequestContext& ctx) {
    ...
    LOG_WARNING(Service_FS, "(STUBBED) FS_USER GetArchiveResource called Media type=0x{:08X}", system_media_type);
