...
System::ResultStatus System::Load(Frontend::EmuWindow& emu_window, const std::string& filepath) {
    ...
    if (title_id == 0x0004000000068B00 || title_id == 0x0004000000061300 || title_id == 0x000400000004A700) {
        ...
        std::string path = FileSys::ArchiveSource_SDSaveData::GetSaveDataPathFor(sdmc_dir,
                                                                                 title_id);
        FileUtil::CreateFullPath(path);
        FileUtil::IOFile(path + "SystemData.bin", "wb");
    } else if (title_id == 0x00040000000B0300) {
        // Fractured Soul
        std::string sdmc_dir = FileUtil::GetUserPath(FileUtil::UserPath::SDMCDir);
        std::string path = FileSys::ArchiveSource_SDSaveData::GetSaveDataPathFor(sdmc_dir,
                                                                                 title_id);
    ...
    const std::array<u64, 12> new3ds_game_ids = {
