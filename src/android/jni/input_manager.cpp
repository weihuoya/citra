...
void InputManager::InitProfile() {
    ...
    std::array<std::string, 4> CirclePadConfigs = {
...
bool InputManager::KeyEvent(int button, float value) {
    ...
    // circle pad
    if (button == mCirclePadKeys[0]) {
        mAnalogs[1] = value * MAX_ANALOG_VALUE;
        return true;
    } else if (button == mCirclePadKeys[1]) {
        mAnalogs[1] = -value * MAX_ANALOG_VALUE;
        return true;
    } else if (button == mCirclePadKeys[2]) {
        mAnalogs[0] = -value * MAX_ANALOG_VALUE;
        return true;
    } else if (button == mCirclePadKeys[3]) {
        mAnalogs[0] = value * MAX_ANALOG_VALUE;
        return true;
    }
