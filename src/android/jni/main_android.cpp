...
JNIEXPORT void JNICALL Java_org_citra_emu_NativeLibrary_searchMemoryRegion(JNIEnv* env, jclass obj,
                                                                           jint region, jint valueType,
                                                                           jint searchType, jint scanType,
                                                                           jint value) {
    // todo
}
...
JNIEXPORT jintArray JNICALL Java_org_citra_emu_NativeLibrary_loadPage(JNIEnv* env, jclass obj, jint index) {
    Core::System& system{Core::System::GetInstance()};
    auto p = system.Memory().GetCurrentPageTable()->pointers[index];
    if (p != nullptr) {
        jintArray array = env->NewIntArray(Memory::PAGE_SIZE / 4);
        env->SetIntArrayRegion(array, 0, Memory::PAGE_SIZE / 4, reinterpret_cast<const int*>(p));
        return array;
    } else {
        return nullptr;
    }
