...
std::unique_ptr<EGLAndroid> EGLAndroid::CreateSharedContext() const {
    ...
    auto new_context = std::make_unique<EGLAndroid>();
    new_context->m_egl_context = new_egl_context;
    new_context->m_egl_display = m_egl_display;
    new_context->m_config = m_config;
    new_context->window_width = window_width;
    new_context->window_height = window_height;
    new_context->m_is_shared = true;
    new_context->UpdateCurrentFramebufferLayout(window_width, window_height);
    return new_context;
}
